...
RegisterCommand('mdt', function()
    ...
    if not IsVisible and IsPedInAnyPoliceVehicle(playerPed) then
        local vehicleInDirection = ESX.Game.GetVehicleInDirection()
        if vehicleInDirection then
            return TriggerServerEvent("mdt:performVehicleSearchInFront", GetVehicleNumberPlateText(vehicleInDirection))
        end
    end
